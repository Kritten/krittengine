...
void main(void) 
{
    ...
    vec3 texture = vec3(texture2D(normal_texture, passed_texture).rgb);
    vec3 texture_normal = normalize(tangentSpace * normalize(vec3(texture.rg * 2.0 - 1.0, texture.b)));
    ...
    for(int i=0; i < MAX_LIGHTS; i++)
    {
        ...
        vec3 fragment_to_light = normalize(light - passed_position);

        // float dot_diffus = dot(norm_normal, fragment_to_light);
        float dot_diffus = dot(texture_normal, fragment_to_light);
        vec4 diffus = diffus_part * max(0.0, dot_diffus);


        float dot_specular= dot(normalize(reflect(fragment_to_light, texture_normal)), normalize(passed_position));
    ...
    // gl_FragColor = vec4(passed_normal, 1.0);  
